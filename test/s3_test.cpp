...
protected:
  static inline bool minio_available_ = false;

  static void SetUpTestCase() {
    minio_available_ = s3cpp::Ping("127.0.0.1:9000");
    if (!minio_available_) return;

    S3Client client =
        S3Client("minio_access", "minio_secret", "127.0.0.1:9000", S3AddressingStyle::PathStyle);

    // Create bucket "my-bucket"
    auto createBucketRes = client.CreateBucket("my-bucket");
    if (!createBucketRes &&
        createBucketRes.error().Code != "BucketAlreadyOwnedByYou") { // Skip if already created
      return;
    }
  ...
  void SetUp() override {
    if (!minio_available_) {
      GTEST_SKIP() << "MinIO is not running. Start it with:\n"
  ...
  static void TearDownTestSuite() {
    ...
    std::vector<std::string> BucketsToDelete{"test-bucket-s3cpp", "test-bucket-location", "test-bucket-tags"};
...
TEST_F(S3, ListObjectsFilePrefix) {
  ...
  std::expected<ListObjectsResult, Error> res =
      client.ListObjects("my-bucket", {.Prefix = "path/to/file_1.txt"});
  if (!res)
    GTEST_FAIL();
...
TEST_F(S3, ListObjectsDirPrefix) {
  ...
  // Get 100 keys
  std::expected<ListObjectsResult, Error> res =
      client.ListObjects("my-bucket", {.MaxKeys = 100, .Prefix = "path/to/"});
  if (!res)
    GTEST_FAIL();
...
TEST_F(S3, ListObjectsDirPrefixMaxKeys) {
  S3Client client("minio_access", "minio_secret", "127.0.0.1:9000", S3AddressingStyle::PathStyle);
  std::expected<ListObjectsResult, Error> res =
      client.ListObjects("my-bucket", {.MaxKeys = 1, .Prefix = "path/to/"});
  if (!res)
    GTEST_FAIL();
  EXPECT_EQ(res->Contents.size(), 1);
}
...
TEST_F(S3, ListObjectsCheckFields) {
  S3Client client("minio_access", "minio_secret", "127.0.0.1:9000", S3AddressingStyle::PathStyle);
  std::expected<ListObjectsResult, Error> res =
      client.ListObjects("my-bucket", {.MaxKeys = 2, .Prefix = "path/to/"});
  if (!res)
    GTEST_FAIL();
...
TEST_F(S3, ListObjectsWithDelimiter) {
  S3Client client("minio_access", "minio_secret", "127.0.0.1:9000", S3AddressingStyle::PathStyle);
  std::expected<ListObjectsResult, Error> res =
      client.ListObjects("my-bucket", {.Delimiter = "/", .Prefix = "path/"});
  if (!res) {
    GTEST_FAIL();
  }
...
TEST_F(S3, ListObjectsWithStartAfter) {
  S3Client client("minio_access", "minio_secret", "127.0.0.1:9000", S3AddressingStyle::PathStyle);
  std::expected<ListObjectsResult, Error> res = client.ListObjects(
      "my-bucket", {.MaxKeys = 10, .Prefix = "path/to/", .StartAfter = "path/to/file_50.txt"});
  if (!res) {
    GTEST_FAIL();
  }
...
TEST_F(S3, ListObjectsWithContinuationToken) {
  S3Client client("minio_access", "minio_secret", "127.0.0.1:9000", S3AddressingStyle::PathStyle);
  std::expected<ListObjectsResult, Error> firstPage =
      client.ListObjects("my-bucket", {.MaxKeys = 10, .Prefix = "path/to/"});
  if (!firstPage) {
    GTEST_FAIL();
  }
  ...
  std::expected<ListObjectsResult, Error> secondPage = client.ListObjects(
      "my-bucket",
      {.ContinuationToken = firstPage->NextContinuationToken, .MaxKeys = 10, .Prefix = "path/to/"});
  if (!secondPage)
    GTEST_FAIL();
...
TEST_F(S3, PutObjectTxt) {
  ...
  if (!PutResponse) {
    FAIL() << std::format("PutObject request failed: Code={}, Message={}", PutResponse.error().Code,
                          PutResponse.error().Message);
  }
  std::expected<std::string, Error> GetResponse = client.GetObject("my-bucket", "some/file.txt");
  if (!GetResponse) {
    FAIL() << std::format("GetObject request failed: Code={}, Message={}", GetResponse.error().Code,
...
TEST_F(S3, CreateBucket) {
  ...
  if (!res) {
    FAIL() << std::format("CreateBucket request failed: Code={}, Message={}", res.error().Code,
                          res.error().Message);
  }

  std::expected<ListObjectsResult, Error> listRes = client.ListObjects(bucketName);
  if (!listRes) {
    FAIL() << std::format("ListObjects after CreateBucket failed: Code={}, Message={}", listRes.error().Code,
...
TEST_F(S3, CreateBucketWithTags) {
  ...
  CreateBucketConfiguration config;
  config.Tags = {{"Environment", "Test"}, {"Project", "s3cpp"}};
  CreateBucketInput options;

  auto res = client.CreateBucket(bucketName, config, options);
  if (!res) {
    FAIL() << std::format("CreateBucket with Tags failed: Code={}, Message={}", res.error().Code,
                          res.error().Message);
  }
...
TEST_F(S3, DeleteBucketWithElements) {
  ...
  if (!putObjectRes)
    FAIL() << std::format("Unable to put object: Code={}, Message={}", putObjectRes.error().Code,
                          putObjectRes.error().Message);
  ;
...
TEST_F(S3, DeleteEmptyBucket) {
  ...
  if (!createBucketRes && createBucketRes.error().Code != "BucketAlreadyOwnedByYou") {
    FAIL() << std::format("DeleteEmptyBucket: Unable to create bucket: Code={}, Message={}",
                          createBucketRes.error().Code, createBucketRes.error().Message);
  }

  // TODO(cristian): Implement overloading of = for client.* calls ???
  auto deleteBucket = client.DeleteBucket("test-empty-bucket");
  if (!deleteBucket) {
    FAIL() << std::format("DeleteBucket failed: Code={}, Message={}", deleteBucket.error().Code,
...
TEST_F(S3, DeleteBucketAndElementsWithPaginator) {
  ...
  for (int i = 0; i < 10; i++) {
    auto putObjectRes =
        client.PutObject("test-bucket-321", std::format("path/to/file{}", i + 1), "Body contents");
    if (!putObjectRes)
      FAIL() << std::format("Unable to put object: Code={}, Message={}", putObjectRes.error().Code,
                            putObjectRes.error().Message);
  }
  ...
  while (paginator.HasMorePages()) {
    ...
    for (const auto &obj : page->Contents) {
      ...
      if (!req)
        FAIL() << std::format("Unable to delete object. Code={}, Message={}", req.error().Code,
                              req.error().Message);
    }
  }
  ...
  if (!deleteBucketRes)
    FAIL() << std::format("Unable to delete bucket. Code={}, Message={}", deleteBucketRes.error().Code,
                          deleteBucketRes.error().Message);
}
...
TEST_F(S3, HeadBucketExists) {
  ...
  if (!res)
    FAIL() << std::format("HeadBucket request failed for an existing bucket. Code={}, Message={}",
                          res.error().Code, res.error().Message);
}
...
TEST_F(S3, HeadBucketNotExists) {
  ...
  if (res.has_value())
    FAIL() << std::format("HeadBucket request succeeded when it should have failed. Code={}, Message={}",
                          res.error().Code, res.error().Message);
...
TEST_F(S3, HeadObjectExists) {
  ...
  if (!res)
    FAIL() << std::format("HeadObject request failed for an existing object. Code={}, Message={}",
                          res.error().Code, res.error().Message);
...
TEST_F(S3, ListBuckets) {
  ...
  if (res) {
    ...
    FAIL() << std::format("ListBuckets request failed. Code={}, Message={}", res.error().Code,
                          res.error().Message);
  }
}

TEST_F(S3, PingEndpoint) {
    EXPECT_TRUE(s3cpp::Ping("s3.us-east-1.amazonaws.com"));
    EXPECT_TRUE(s3cpp::Ping("127.0.0.1:9000"));
